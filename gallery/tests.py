...
class TestImage(TestCase):
    def setUp(self):
        self.location = Location(name='Carlifornia')
        self.location.save_location()

        self.category = Category(name='nature')
        self.category.save_category()

        self.image_test = Image(id=1, name='image', description='Beautiful scenery', location=self.location,
    ...
    def test_search_image_by_location(self):
        self.image_test.save_image()
        found_images = self.image_test.filter_by_location(location='Carlifornia')
...
class TestLocation(TestCase):
    ...
    def test_update_location(self):
        new_location = 'Michigan'
        self.location.update_location(self.location.id, new_location)
        changed_location = Location.objects.filter(name='Michigan')
...
class CategoryTestClass(TestCase):
    def setUp(self):
        self.category = Category(name='Architecture')
        self.category.save_category()
